...
public class LifecycleApp {
	...
	/*
		LifecycleBean objA = (LifecycleBean) context.getBean("lifecyclebean");
		System.out.println("LifecycleBean:"+objA.getMessage());
		 ((AbstractApplicationContext) context).registerShutdownHook();
		 
   */
		
		 context = new AnnotationConfigApplicationContext(MySpringConfiguration.class);
		MySpringBean obj = (MySpringBean) context.getBean("myspringbean");
		System.out.println("MySpringBean"+obj.getMessage());
		 ((AbstractApplicationContext) context).registerShutdownHook();
